...
public class OrderHandleServiceImpl implements OrderHandleService {
  ...
  @Resource private ShoppingCartMapper shoppingCartMapper;
  @Resource private BookMapper bookMapper;
  @Resource private UserMapper userMapper;
  @Resource private CreditLevelMapper creditLevelMapper;
  ...
  public void createOrder(Order order) {
    ...
    if (creditLevel != null) {
      discountRate =
          creditLevel.getDiscountRate() != null ? creditLevel.getDiscountRate() : BigDecimal.ZERO;
      overdraftLimit =
          creditLevel.getOverdraftLimit() != null
              ? creditLevel.getOverdraftLimit()
              : BigDecimal.ZERO;
    }
    ...
    for (OrderItem orderItem : order.getOrderItems()) {
      ...
      if (stock < orderItem.getQuantity()) {
        throw new CustomizeException(
            ResultCode.INSUFFICIENT_STOCK, "库存不足，bookId=" + orderItem.getBookId());
      }
      // 按折扣计算单价与小计（四舍五入到2位）
    ...
    BigDecimal userBalance =
        (user != null && user.getAccountBalance() != null)
    ...
    for (OrderItem orderItem : order.getOrderItems()) {
      int updated =
          bookMapper.decrementStockIfEnough(orderItem.getBookId(), orderItem.getQuantity());
      if (updated == 0) {
        throw new CustomizeException(
            ResultCode.INSUFFICIENT_STOCK, "库存不足或已被其他订单占用，bookId=" + orderItem.getBookId());
      }
    }
    // 插入订单子项并删除购物车记录
    for (OrderItem orderItem : order.getOrderItems()) {
  ...
  public void shipOrder(Integer orderId, List<OrderItem> shipItems) {
    ...
    for (OrderItem shipItem : shipItems) {
      ...
      int toShip = shipItem.getQuantity() == null ? 0 : shipItem.getQuantity();
      int newShipped =
          (exist.getShippedQuantity() == null ? 0 : exist.getShippedQuantity()) + toShip;
