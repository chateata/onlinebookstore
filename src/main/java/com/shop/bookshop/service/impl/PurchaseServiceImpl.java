...
public class PurchaseServiceImpl implements PurchaseService {
  ...
  @Resource private PurchaseOrderMapper purchaseOrderMapper;
  @Resource private PurchaseOrderItemMapper purchaseOrderItemMapper;
  @Resource private BookMapper bookMapper;
  @Resource private ShortageMapper shortageMapper;
  @Resource private SupplierBookMapper supplierBookMapper;
  @Resource private com.shop.bookshop.dao.PublisherMapper publisherMapper;
  ...
  public Integer createPurchaseOrder(PurchaseOrder purchaseOrder) {
    ...
    for (PurchaseOrderItem item : purchaseOrder.getItems()) {
      if (item.getUnitPrice() == null) {
        ...
        if (item.getSupplierBookId() != null) {
          SupplierBook supplierBook =
              supplierBookMapper.selectBySupplierBookId(item.getSupplierBookId());
      ...
      BigDecimal line =
          item.getUnitPrice().multiply(new BigDecimal(qty)).setScale(2, BigDecimal.ROUND_HALF_UP);
  ...
  public void receivePurchaseOrderItem(Integer poItemId, Integer addedReceived) {
    ...
    int toAdd =
        (addedReceived == null) ? remaining : Math.min(Math.max(0, addedReceived), remaining);
    if (toAdd <= 0) {
    ...
    if (poi.getSupplierBookId() != null) {
      SupplierBook supplierBook =
          supplierBookMapper.selectBySupplierBookId(poi.getSupplierBookId());
      if (supplierBook != null) {
        ...
        if (existingBook == null) {
          ...
          existingBook.setStock(
              (existingBook.getStock() == null ? 0 : existingBook.getStock()) + toAdd);
